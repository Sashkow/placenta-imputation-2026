...
phase2b_dir <- "scripts/lib"
source(file.path(phase2b_dir, "imputation.R"))
source(file.path(phase2b_dir, "normalization.R"))

if (!exists("compute_ga_weeks")) {
  GA_TRIMESTER_MIDPOINTS <- c(
    "First Trimester" = 8, "Second Trimester" = 20,
    "Third Trimester" = 39, "Term" = 39,
    "Late Preterm" = 35, "Early Preterm" = 30,
    "First trimester" = 8, "Second trimester" = 20,
    "Third trimester" = 39, "Late preterm" = 35,
    "Early preterm" = 30
  )
  compute_ga_weeks <- function(pdata) {
    ga_weeks <- rep(NA_real_, nrow(pdata))
    has_fw <- "fetus_week" %in% colnames(pdata)
    has_fr <- "fetus_range_week" %in% colnames(pdata)
    has_ga <- "Gestational.Age" %in% colnames(pdata)
    has_bounds <- all(c("Gestational.Age.Lower.Bound",
                        "Gestational.Age.Upper.Bound") %in% colnames(pdata))
    ga_cat_col <- if ("Gestational.Age.Category" %in% colnames(pdata)) {
      "Gestational.Age.Category"
    } else if ("trimester" %in% colnames(pdata)) "trimester" else NULL
    for (i in seq_len(nrow(pdata))) {
      if (has_fw) {
        val <- suppressWarnings(as.numeric(pdata$fetus_week[i]))
        if (!is.na(val) && val > 0) { ga_weeks[i] <- val; next }
      }
      if (has_ga) {
        val <- suppressWarnings(as.numeric(as.character(pdata$Gestational.Age[i])))
        if (!is.na(val) && val > 0) { ga_weeks[i] <- val; next }
      }
      if (has_fr) {
        rw <- as.character(pdata$fetus_range_week[i])
        if (!is.na(rw) && nzchar(rw) && rw != "0") {
          opts <- suppressWarnings(as.numeric(unlist(strsplit(rw, ","))))
          opts <- opts[!is.na(opts) & opts > 0]
          if (length(opts) > 0) { ga_weeks[i] <- median(opts); next }
        }
      }
      if (has_bounds) {
        lo <- suppressWarnings(as.numeric(pdata$Gestational.Age.Lower.Bound[i]))
        hi <- suppressWarnings(as.numeric(pdata$Gestational.Age.Upper.Bound[i]))
        if (!is.na(lo) && !is.na(hi) && lo > 0 && hi > 0) {
          ga_weeks[i] <- (lo + hi) / 2; next
        }
      }
      if (!is.null(ga_cat_col)) {
        cat_val <- as.character(pdata[[ga_cat_col]][i])
        if (cat_val %in% names(GA_TRIMESTER_MIDPOINTS))
          ga_weeks[i] <- GA_TRIMESTER_MIDPOINTS[[cat_val]]
      }
    }
    ga_weeks
  }
...
run_lean_pipeline_inner <- function(exprs_sub_list, pdata_sub, config,
  ...
  combat_bio_degree <- config$phenotype$combat_bio_degree %||% 1L
  combat_bio_spline_df <- config$phenotype$combat_bio_spline_df
  if (!is.null(combat_bio_cov)) {
    ...
    combat_mod_data <- data.frame(ga_cont = merged_pdata[[combat_bio_cov]],
                                  stringsAsFactors = FALSE)
    if (!is.null(combat_bio_spline_df)) {
      if (!requireNamespace("splines", quietly = TRUE)) library(splines)
      bio_term <- sprintf("splines::ns(ga_cont, df = %d)", combat_bio_spline_df)
    } else if (combat_bio_degree > 1) {
      bio_term <- sprintf("poly(ga_cont, %d)", combat_bio_degree)
    } else {
      bio_term <- "ga_cont"
    }
  } else {
  ...
  combat_mod_rhs <- paste(c(bio_term, covariates), collapse = " + ")

  # Drop covariates with single level
  for (cov in covariates) {
    if (length(unique(na.omit(combat_mod_data[[cov]]))) < 2) {
      combat_mod_data[[cov]] <- NULL
      combat_mod_rhs <- paste(setdiff(c(bio_term, covariates), cov), collapse = " + ")
